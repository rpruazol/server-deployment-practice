...
describe("basic server functionality", () => {
  ...
  test("request returns a valid timestamp", async () => {
    ...
    expect(response.body.timestamp.slice(0, 19)).toStrictEqual(
      time.slice(0, 19)
    );
  });
  ...
  test("POST a new car", async () => {
    ...
    test("UPDATE a single car", async () => {
      const response = await request
        .put("/updatecar/2")
        .send({ model: "highlander" });
      expect(JSON.parse(response.text).model).toBe("highlander");
    }),
    test("DELETE a single car", async () => {
      let newRequest = await request.get("/cars");
      console.log(newRequest._body);
      expect(newRequest._body.length).toBe(2);
      const response = await request.delete("/deletecar/2");
      expect(response.status).toBe(204);
      expect(response.body).toEqual({});
      console.log(newRequest._body);
      newRequest = await request.get("/cars");
